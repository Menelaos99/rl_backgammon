...
class Game:
    ...
    def select(self, lane, vpos):
        if self.selected:
            print('selected in select', self.selected)
    ...
    def _move(self, lane, vpos):
        ...
        print('piece', piece)
        print(self.board.counts[lane-1])
        print((isinstance(piece, Puli) and self.board.counts[lane-1]>=6))
        print('sel', self.selected)
        print('valid', self.valid_lanes)
        print('valid', (lane, vpos) in self.valid_lanes, lane, vpos)
        if self.selected \
            ...
            return False
        print(self.board.board_storage)
        return True
